...
window.addGoogleFonts = (googleFontsList, opts) => {
  if(typeof googleFontsList !== "undefined"){
    if(googleFontsList.trim() !== ""){
      ...
      fetch("https://glen-api.gitlab.io/public/gfonts-latest.json")
      ...
      .then(res => {
              ...
              /*---- IS NOT A VARIABLE FONT ----*/
                ...
                if(wghts){
                  if(Array.isArray(wghts)){
                    ...
                    if(opts && (opts.logs == true || opts.logs == "true")){
                      console.info(`${font.replaceAll("+"," ")}: ${wghts.join(", ")} only`);
                    }
        ...
        if(finalFontsArray.length){
          let finalStr = `https://${opts && opts.bunnyFonts === true ? "fonts.bunny.net" : "fonts.googleapis.com"}/css2?family=${finalFontsArray.join("&family=")}&display=swap`
          opts && opts.bunnyFonts === true && console.info(`Generated fonts stylesheet link: ${finalStr}`)
